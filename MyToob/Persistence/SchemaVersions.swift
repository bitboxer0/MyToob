...
/// The embedding dimension change from the original Core ML model (384-dim) to
/// Apple NLEmbedding (512-dim) is a semantic change only. The underlying Data
/// storage format remains the same, so no schema migration is required. Existing
/// embeddings will be cleared and regenerated when the EmbeddingService is used.
